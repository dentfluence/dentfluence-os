...
class TimelineController extends Controller
{
    ...
    // ─────────────────────────────────────────────
    // SAMPLE DATA — NOT YET WIRED TO LIVE RECORDS
    // The views currently show a "Preview" banner because of this.
    //
    // TODO (supervised, needs DB + schema verification):
    //   getDummyPatients()  → union of Lead (lead_source/stage/assigned_to)
    //                         and Patient, ordered by latest activity.
    //   getDummyTimeline()  → merge per-person events from:
    //                         lead_activities, communication_queue,
    //                         patient_communications, follow_ups,
    //                         (and appointments/opportunities) into one
    //                         time-sorted feed matching the array shape below.
    //   Keep the exact keys the Blade expects:
    //     patient: id,name,phone,type,status,treatment,avatar,last_activity,assigned_to
    //     event:   id,type,subtype,title,outcome,description,date,time,actor,duration,icon,color
    //   Remove the Preview banners in the two timeline views once live.
    // ─────────────────────────────────────────────
