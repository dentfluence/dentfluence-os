...
class LabController extends Controller
{
    ...
    {
        // Shared brain — same date stamping, task chain, notification and
        // finance-expense side effects as the mobile API path.
        try {
            $task = app(\App\Services\LabCaseTransitionService::class)
                ->transition($labCase, $to, auth()->user());
        ...
        return back()->with('success', "{$labCase->case_number} moved to {$label}." . ($task ? ' Task assigned.' : ''));
    }
