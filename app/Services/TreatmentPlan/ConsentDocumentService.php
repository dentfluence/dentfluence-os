...
class ConsentDocumentService
{
     ...
     * skipping any item not flagged consent_required. Read-only — does not
     * persist anything. Used both for the live print view and as the
     * payload snapshotted by generateAndPersist().
     */
    public function buildSections(TreatmentPlan $plan): array
    {
     ...
     */
    public function generateAndPersist(TreatmentPlan $plan, ?int $userId): TreatmentConsent
    {
        return TreatmentConsent::create([
            'treatment_plan_id' => $plan->id,
            'patient_id'        => $plan->patient_id,
            'generated_by'      => $userId,
            'sections'          => $this->buildSections($plan),
        ]);
    }
}
