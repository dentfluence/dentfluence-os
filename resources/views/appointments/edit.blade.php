...
@section('content')
<div style="padding:28px;font-family:'DM Sans',sans-serif;max-width:700px;margin:0 auto;">
    ...
    <form action="{{ route('appointments.update', $appointment) }}" method="POST"
        ...
        <div style="margin-bottom:16px;">
            <label style="font-size:12px;font-weight:600;color:#4b0e59;display:block;margin-bottom:4px;">Notes</label>
            <textarea name="notes" rows="3" style="width:100%;padding:8px 10px;border:1px solid #d1d5db;border-radius:6px;font-size:13px;">{{ $appointment->notes }}</textarea>
        </div>

        <div style="display:flex;gap:10px;justify-content:flex-end;">
            <a href="{{ route('appointments.index') }}"
               style="padding:9px 18px;border:1px solid #d1d5db;border-radius:6px;font-size:13px;color:#374151;text-decoration:none;">Cancel</a>
            <button type="submit"
                    style="padding:9px 20px;background:#6a0f70;color:#fff;border:none;border-radius:6px;font-size:13px;font-weight:600;cursor:pointer;">
                Save Changes
            </button>
        </div>
    </form>
