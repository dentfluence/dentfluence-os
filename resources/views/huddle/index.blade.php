...
/* ── Escape layout inner padding (huddle is full-bleed) ── */
.hd-escape { margin: -28px -32px -48px; }
@media(max-width:767px){ .hd-escape { margin: -20px -16px -40px; } }

/* ── Fight Tailwind CDN preflight which sets svg{display:block} ── */
.hd svg {
...
.hd-card {
    ...
    border-radius: 14px;
    overflow: hidden;
    box-shadow: 0 1px 5px rgba(56,7,64,.07);
}
.hd-ch {
    display: flex;
    align-items: center;
    padding: .82rem 1.1rem .7rem;
...
.hd-pr {
    ...
    border-bottom: 1px solid #f4eaf9;
    font-size: .81rem;
}
...
.hd-p-sent       { background:#fef3c7; color:#92400e; }
.hd-p-in_progress{ background:#dbeafe; color:#1e40af; }
.hd-p-received   { background:#dcfce7; color:#166534; }
...
.hd-tab {
    padding:.35rem .8rem; font-size:.74rem; font-weight:600;
    border-radius:6px 6px 0 0; border:1px solid transparent; border-bottom:none;
    cursor:pointer; background:transparent; color:var(--ts); transition:background .12s;
}
...
.hd-tdot-wins     { background:#16a34a; }
.hd-tdot-lows     { background:#d97706; }
.hd-tdot-failures { background:#dc2626; }
.hd-tdot-concerns { background:#7c3aed; }
...
/* ── Coming soon ── */
.hd-soon { display:flex; align-items:center; justify-content:center; gap:.5rem; padding:1.4rem; font-size:.78rem; color:var(--ts);
    background:repeating-linear-gradient(45deg,transparent,transparent 8px,rgba(106,15,112,.025) 8px,rgba(106,15,112,.025) 16px);
...
<div class="hd-qa">
    ...
    <a href="{{ route('patients.create') }}">
        <svg style="display:inline-block!important;flex-shrink:0;width:12px;height:12px;max-width:none;vertical-align:middle;" width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
        New Patient
    </a>
    ...
    <a href="{{ route('patients.index') }}">
        <svg style="display:inline-block!important;flex-shrink:0;width:12px;height:12px;max-width:none;vertical-align:middle;" width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
        Find Patient
    </a>
</div>
...
<div class="hd-body">
    ...
    <div>
        <div class="hd-sec">
            <svg style="display:inline-block!important;flex-shrink:0;width:16px;height:16px;max-width:none;vertical-align:middle;" width="16" height="16" fill="none" stroke="#6a0f70" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
        ...
        <div class="hd-3col">
            ...
            <div class="hd-card">
                <div class="hd-ch">
                    <div class="hd-ch-ico">
                        <svg style="display:inline-block!important;flex-shrink:0;width:13px;height:13px;max-width:none;vertical-align:middle;" width="13" height="13" fill="none" stroke="#6a0f70" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                    </div>
                ...
                <div class="hd-pr"
                     x-data="{
                        ...
                        async save(){
                            this.saving=true;
                            await fetch('{{ route('huddle.updateInstruction',$appt->id) }}',{
                                method:'PATCH',
                                headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').content},
                                body:JSON.stringify({staff_instruction:this.instruction})
                            });
                            this.original=this.instruction; this.editing=false; this.saving=false;
                        }
                     }">
                    ...
                    <div style="flex:1;min-width:0;">
                        ...
                        <span class="hd-pr-doc">{{ optional($appt->doctor)->name ?? '—' }}</span>
                    </div>
                    <div style="min-width:80px;max-width:120px;">
                        <div x-show="!editing" @click="editing=true" class="hd-instr" x-text="instruction || ''"></div>
                        <template x-if="editing">
                            <div>
                                <input x-model="instruction" @keydown.enter="save()" @keydown.escape="instruction=original;editing=false"
                                       class="hd-instr-inp" placeholder="Add note…" x-init="$el.focus()" :disabled="saving">
                                <span @click="save()" style="font-size:.67rem;color:var(--dm);cursor:pointer;text-decoration:underline;" x-text="saving?'Saving…':'Save'"></span>
                            </div>
                        </template>
                    </div>
                </div>
            ...
            <div class="hd-card">
                <div class="hd-ch">
                    <div class="hd-ch-ico">
                        <svg style="display:inline-block!important;flex-shrink:0;width:13px;height:13px;max-width:none;vertical-align:middle;" width="13" height="13" fill="none" stroke="#6a0f70" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/></svg>
                    </div>
                    <h3>Lab Due Today</h3>
                ...
                <div class="hd-pr">
                    <div style="flex:1;min-width:0;">
                        ...
                        <div style="font-size:.73rem;color:var(--ts);">{{ $lab->first_name }} {{ $lab->last_name }}</div>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-size:.72rem;color:var(--ts);margin-bottom:.18rem;">{{ $lab->due_time ? \Carbon\Carbon::parse($lab->due_time)->format('H:i') : '—' }}</div>
                        <span class="hd-p hd-p-{{ $lab->status }}">{{ ucfirst(str_replace('_',' ',$lab->status)) }}</span>
                    </div>
                </div>
                @empty
                <div class="hd-soon">
                    <svg style="display:inline-block!important;flex-shrink:0;width:13px;height:13px;max-width:none;vertical-align:middle;" width="13" height="13" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            ...
            <div class="hd-card">
                <div class="hd-ch">
                    <div class="hd-ch-ico" style="background:#fff0f0;">
                        <svg style="display:inline-block!important;flex-shrink:0;width:13px;height:13px;max-width:none;vertical-align:middle;" width="13" height="13" fill="none" stroke="#c53030" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                    </div>
                ...
                <div style="padding:.75rem .9rem;">
                    ...
                    <div class="hd-al hd-al-{{ $alert['level'] }}">
                        <svg style="display:inline-block!important;flex-shrink:0;width:14px;height:14px;max-width:none;vertical-align:middle;" width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="hd-al-clear">
                        <svg style="display:inline-block!important;flex-shrink:0;width:28px;height:28px;max-width:none;vertical-align:middle;" width="28" height="28" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
    ...
    <div>
        ...
        <div class="hd-2col">
            ...
            <div class="hd-card"
                 x-data="{
                    ...
                    async addNote(){
                        ...
                        const res=await fetch('{{ route('huddle.storeNote') }}',{
                            method:'POST',
                            headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').content},
                            body:JSON.stringify({category:this.activeTab,body:this.newNote,date:'{{ $today->toDateString() }}'})
                        });
                        const d=await res.json();
                        this.notes[this.activeTab].push({body:d.body,author:{name:d.author},created_at_formatted:d.created_at});
                        this.newNote='';this.submitting=false;
                    }
                 }">
                <div class="hd-tabs">
                ...
                <div class="hd-nform">
                    <input x-model="newNote" class="hd-ninp" :placeholder="'Add a '+activeTab+' note…'" @keydown.enter="addNote()" :disabled="submitting">
            ...
            {{-- My Tasks --}}
            <div class="hd-card"
                 x-data="{tasks:@json($myTasks->values()),toggle(i){this.tasks[i].done=!this.tasks[i].done;}}">
                <div class="hd-ch">
                    <div class="hd-ch-ico">
                        <svg style="display:inline-block!important;flex-shrink:0;width:13px;height:13px;max-width:none;vertical-align:middle;" width="13" height="13" fill="none" stroke="#6a0f70" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>
                    </div>
                ...
                <template x-if="tasks.length===0">
                    <div class="hd-soon">
                        <svg style="display:inline-block!important;flex-shrink:0;width:12px;height:12px;max-width:none;vertical-align:middle;" width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                ...
                <template x-for="(task,i) in tasks" :key="task.id">
                    <div class="hd-tr">
                        <div class="hd-tck" :class="{done:task.done}" @click="toggle(i)">
                            <svg style="display:inline-block!important;flex-shrink:0;width:8px;height:8px;max-width:none;vertical-align:middle;" width="8" height="8" fill="none" stroke="white" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
                        </div>
        ...
        <div class="hd-card"
             x-data="{
                ...
                markDone(id,i){
                    ...
                    fetch('/communication-entries/'+id+'/done',{method:'PATCH',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').content}});
                }
             }">
            ...
            <template x-for="(item,i) in items" :key="item.id">
                <div class="hd-cr">
                    ...
                    <div style="flex:1;min-width:0;">
                        <div style="font-weight:500;font-size:.81rem;" x-text="item.first_name+' '+item.last_name"></div>
                        <div style="font-size:.72rem;color:var(--ts);" x-text="item.phone"></div>
                        <div x-show="item.notes" x-text="item.notes" style="font-size:.71rem;color:var(--ts);"></div>
                    </div>
