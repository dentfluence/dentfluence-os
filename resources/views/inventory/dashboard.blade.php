...
| Added: Health Score hero, extra KPIs, role-conditional views
|==========================================================================
--}}
@extends('layouts.app')
...
<style>
    ...
    .inv-health-score-num {
        font-size: 24px; font-weight: 800; line-height: 1;
        color: var(--health-color); font-family: 'Inter', sans-serif;
    }
    .inv-health-score-label {
        font-size: 9px; font-weight: 600; color: #9a85aa;
    ...
    @media (max-width: 1100px) { .inv-extra-kpi-grid { grid-template-columns: repeat(3, 1fr); } }
    @media (max-width: 640px)  { .inv-extra-kpi-grid { grid-template-columns: repeat(2, 1fr); } }

    /* ── Phase 2: Assistant Action Cards ── */
    .inv-action-list { display: flex; flex-direction: column; gap: 8px; margin-bottom: 20px; }
    .inv-action-item {
    ...
    .inv-action-item a.inv-action-btn {
        margin-left: auto; flex-shrink: 0;
        display: inline-flex; align-items: center; gap: 4px;
        padding: 5px 12px; border-radius: 4px;
        background: #f3e8ff; color: #6a0f70;
        font-size: 12px; font-weight: 500; text-decoration: none;
        border: 1px solid #e0d0ea;
        white-space: nowrap;
    }
    .inv-action-item a.inv-action-btn:hover { background: #e9d5f5; }

    /* ── Phase 2: Doctor exec view ── */
    .inv-exec-grid {
        display: grid;
        grid-template-columns: 1fr 1fr;
        gap: 16px;
    ...
    @media (max-width: 700px) { .inv-exec-grid { grid-template-columns: 1fr; } }
    .inv-exec-card {
        background: #fff; border: 1px solid #ede4f0; border-radius: 8px; padding: 20px 24px;
    }
    .inv-exec-card .exec-num {
        font-size: 32px; font-weight: 800; line-height: 1; margin-bottom: 4px;
        font-family: 'Inter', sans-serif;
    }
    .inv-exec-card .exec-label { font-size: 13px; font-weight: 500; color: #4a3a5a; margin-bottom: 2px; }
    .inv-exec-card .exec-sub   { font-size: 11.5px; color: #9a85aa; }
</style>
@endsection
...
@php
    // Role tier detection for conditional rendering
    $dashUser      = auth()->user();
    $dashRole      = $dashUser?->role ?? '';
    $dashIsDoctor  = in_array($dashRole, \App\Models\User::DOCTOR_ROLES);
    $dashIsAssist  = in_array($dashRole, ['assistant', 'front_desk']);
    $dashIsManager = in_array($dashRole, ['manager', 'accounts']) || $dashUser?->isAdmin();
@endphp

{{-- ════════════════════════════════════════════════════════════
     HEALTH SCORE HERO (visible to all roles)
════════════════════════════════════════════════════════════ --}}
<div class="inv-health-hero"
     style="--health-bg: {{ $healthScore['bg'] }};
            --health-border: {{ $healthScore['border'] }};
            --health-color: {{ $healthScore['color'] }};">

    {{-- Score ring --}}
    <div class="inv-health-score-ring">
        <div class="inv-health-score-num">{{ $healthScore['score'] }}</div>
        <div class="inv-health-score-label">/ 100</div>
    </div>

    {{-- Grade + breakdown --}}
    <div style="flex:1;min-width:0;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;flex-wrap:wrap;">
            <span style="font-size:18px;font-weight:700;color:{{ $healthScore['color'] }};font-family:'Inter',sans-serif;">
                Inventory Health: {{ $healthScore['grade'] }}
            </span>
            <span style="font-size:11px;color:#9a85aa;">
                Updated {{ now()->format('d M, h:i A') }}
            </span>
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:4px;">
            @if($healthScore['penalties']['oos'] > 0)
            <span class="inv-health-penalty" style="color:#dc2626;">
                <span style="width:6px;height:6px;border-radius:50%;background:#dc2626;display:inline-block;"></span>
                {{ $healthScore['penalties']['oos'] }} out of stock
            </span>
            @endif
            @if($healthScore['penalties']['low'] > 0)
            <span class="inv-health-penalty" style="color:#d97706;">
                <span style="width:6px;height:6px;border-radius:50%;background:#d97706;display:inline-block;"></span>
                {{ $healthScore['penalties']['low'] }} low stock
            </span>
            @endif
            @if($healthScore['penalties']['expiry'] > 0)
            <span class="inv-health-penalty" style="color:#b45309;">
                <span style="width:6px;height:6px;border-radius:50%;background:#b45309;display:inline-block;"></span>
                {{ $healthScore['penalties']['expiry'] }} expiring soon
            </span>
            @endif
            @if($healthScore['penalties']['dead'] > 0)
            <span class="inv-health-penalty" style="color:#6b7280;">
                <span style="width:6px;height:6px;border-radius:50%;background:#6b7280;display:inline-block;"></span>
                {{ $healthScore['penalties']['dead'] }} dead stock
            </span>
            @endif
            @if($healthScore['score'] >= 90)
            <span class="inv-health-penalty" style="color:#15803d;">
    ...
    <a href="{{ route('inventory.alerts') }}"
       style="flex-shrink:0;display:inline-flex;align-items:center;gap:6px;
              padding:8px 16px;background:#fff;border:1px solid {{ $healthScore['border'] }};
              border-radius:6px;font-size:13px;font-weight:500;color:{{ $healthScore['color'] }};
...
{{-- ════════════════════════════════════════════════════════════
     DENTIST / OWNER VIEW — exec KPIs only, stop here
════════════════════════════════════════════════════════════ --}}
@if($dashIsDoctor)
...
<div class="inv-exec-grid">
    ...
    <div class="inv-exec-card">
        <div class="exec-num" style="color:#6a0f70;">{{ $kpis[0]['value'] }}</div>
        <div class="exec-label">Total Inventory Value</div>
        <div class="exec-sub">Stock on hand at cost price</div>
    </div>
    {{-- Total Items --}}
    <div class="inv-exec-card">
        <div class="exec-num" style="color:#1a5ea8;">{{ $kpis[1]['value'] }}</div>
        <div class="exec-label">Active Items</div>
        <div class="exec-sub">Items in the catalogue</div>
    </div>
    {{-- Dead Stock --}}
    <div class="inv-exec-card">
        <div class="exec-num" style="color:{{ $extraKpis[1]['color'] }};">{{ $extraKpis[1]['value'] }}</div>
        <div class="exec-label">Dead Stock Value</div>
        <div class="exec-sub">{{ $extraKpis[1]['insight'] }}</div>
    </div>
    {{-- Monthly Wastage --}}
    <div class="inv-exec-card">
        <div class="exec-num" style="color:{{ $extraKpis[2]['color'] }};">{{ $extraKpis[2]['value'] }}</div>
        <div class="exec-label">Monthly Wastage</div>
        <div class="exec-sub">{{ $extraKpis[2]['insight'] }}</div>
    </div>
</div>

@php /* Doctors see nothing below — stop rendering */ @endphp
@else {{-- Not a doctor — render full dashboard --}}

{{-- ════════════════════════════════════════════════════════════
     ASSISTANT / FRONT DESK — action cards first
════════════════════════════════════════════════════════════ --}}
@if($dashIsAssist && count($actionItems) > 0)
<div style="margin-bottom:20px;">
    <div style="font-size:12px;font-weight:600;color:#6a0f70;text-transform:uppercase;
    ...
    <div class="inv-action-list">
        ...
        @php
            $actionColor = $action['priority'] === 'high' ? '#dc2626' : '#d97706';
        @endphp
        <div class="inv-action-item" style="--action-color: {{ $actionColor }};">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                 stroke="{{ $actionColor }}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                 style="flex-shrink:0;">
                <path d="{{ $action['icon'] }}"/>
            </svg>
            <div style="flex:1;min-width:0;">
                <div style="font-size:13px;font-weight:500;color:#1e0a2c;">{{ $action['text'] }}</div>
                <div style="font-size:11.5px;color:#9a85aa;margin-top:1px;">{{ $action['sub'] }}</div>
            </div>
            <a href="{{ $action['href'] }}" class="inv-action-btn">{{ $action['action'] }}</a>
        </div>
        @endforeach
    </div>
</div>
...
<div class="inv-kpi-grid">
    ...
    <div class="inv-kpi-card {{ !empty($kpi['alert']) ? 'alert-card' : '' }}"
        ...
        {{-- Value ── --}}
        <div style="font-family:'Inter', sans-serif;font-size:22px;font-weight:600;color:{{ $kpi['color'] }};line-height:1;margin-bottom:4px;">
            {{ $kpi['value'] }}
        </div>
        {{-- Label ── --}}
...
{{-- ════════════════════════════════════════════════════════════
     1b. EXTRA KPI ROW (Phase 2) — Days Until Stockout, Dead Stock, Wastage, Deliveries, Implants
════════════════════════════════════════════════════════════ --}}
<div class="inv-extra-kpi-grid">
    @foreach($extraKpis as $kpi)
    <div class="inv-kpi-card {{ !empty($kpi['alert']) ? 'alert-card' : '' }}"
         style="--alert-color: {{ $kpi['color'] }};background:{{ $kpi['bg'] }};">
        <div style="font-family:'Inter',sans-serif;font-size:20px;font-weight:700;color:{{ $kpi['color'] }};line-height:1;margin-bottom:4px;">
            {{ $kpi['value'] }}
        </div>
        <div style="font-size:12px;font-weight:500;color:#1e0a2c;margin-bottom:3px;">{{ $kpi['label'] }}</div>
        <div style="font-size:11px;color:#9a85aa;">{{ $kpi['insight'] }}</div>
        @if(!empty($kpi['alert']))
        <div style="position:absolute;top:10px;right:10px;width:7px;height:7px;background:{{ $kpi['color'] }};border-radius:50%;"></div>
        @endif
    </div>
        ...
        <div class="inv-card-body" style="display:flex;align-items:center;gap:20px;">
            <div style="position:relative;width:110px;height:110px;flex-shrink:0;">
                ...
                <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;pointer-events:none;">
                    <div style="font-size:18px;font-weight:700;color:#1e0a2c;font-family:'Inter', sans-serif;">{{ $total }}</div>
            ...
            <div style="flex:1;">
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                    ...
                    <span style="font-size:12px;font-weight:600;color:#1e0a2c;font-family:'Inter', sans-serif;">{{ $s['value'] }}</span>
                </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="inv-card-body" style="padding-top:12px;">
            ...
            <div class="inv-bar-row">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    ...
                    <span style="font-size:11px;color:#9a85aa;font-family:'Inter', sans-serif;">{{ $cv['label'] }}</span>
                </div>
            ...
            <table style="width:100%;border-collapse:collapse;font-size:12.5px;">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid rgba(185,92,183,0.05);transition:background 120ms;" onmouseover="this.style.background='#faf5fb'" onmouseout="this.style.background=''">
                        ...
                        <td style="padding:10px 14px;text-align:center;font-family:'Inter', sans-serif;font-weight:600;color:{{ $item->total_stock <= 0 ? '#b52020' : '#a05c00' }};">
                            {{ number_format($item->total_stock, 0) }}
                        </td>
                        <td style="padding:10px 14px;text-align:center;font-family:'Inter', sans-serif;color:#9a85aa;">
            ...
            <table style="width:100%;border-collapse:collapse;font-size:12.5px;">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid rgba(185,92,183,0.05);transition:background 120ms;" onmouseover="this.style.background='#faf5fb'" onmouseout="this.style.background=''">
                        ...
                        <td style="padding:10px 14px;font-family:'Inter', sans-serif;font-size:11.5px;color:#4e2060;">
                            {{ $exp->batch_no ?: '—' }}
                        </td>
                        <td style="padding:10px 18px;">
    ...
    <div class="inv-movement-feed">
        ...
        <div class="inv-movement-item">
            ...
            <div style="text-align:right;flex-shrink:0;">
                <div style="font-family:'Inter', sans-serif;font-size:14px;font-weight:600;color:{{ $tc['color'] }};">
...
<div class="inv-footer-strip">
    ...
    <div class="inv-footer-stat">
        <div style="font-family:'Inter', sans-serif;font-size:18px;font-weight:700;color:#1e0a2c;line-height:1;">{{ $stat['value'] }}</div>
...
<div id="inv-reports" style="margin-top:32px;scroll-margin-top:80px;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
        ...
        <div style="background:#fff;border:1px solid rgba(185,92,183,0.10);border-radius:4px;overflow:hidden;">
            <div style="background:#faf5fb;padding:12px 18px;border-bottom:1px solid rgba(185,92,183,0.08);
                ...
                <span style="font-family:'Inter',sans-serif;font-size:12.5px;font-weight:600;
                             text-transform:uppercase;letter-spacing:0.04em;color:#4e0a53;">
                    Stock Valuation by Category
                </span>
                <span style="font-family:'Inter',sans-serif;font-size:13px;font-weight:700;color:#6a0f70;">
                ...
                <div style="margin-bottom:11px;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
                        <div style="display:flex;align-items:center;gap:7px;">
                            <span style="width:8px;height:8px;border-radius:50%;background:{{ $cv->color ?: '#9070a0' }};flex-shrink:0;display:inline-block;"></span>
                            <span style="font-family:'Inter',sans-serif;font-size:12.5px;color:#2e1040;">{{ $cv->name }}</span>
                            <span style="font-family:'Inter',sans-serif;font-size:11px;color:#c0b0cc;">{{ $cv->item_count }} items</span>
                        </div>
                        <span style="font-family:'Inter',sans-serif;font-size:12px;color:#4e0a53;font-weight:600;">
        ...
        <div style="background:#fff;border:1px solid rgba(185,92,183,0.10);border-radius:4px;overflow:hidden;">
            <div style="background:#faf5fb;padding:12px 18px;border-bottom:1px solid rgba(185,92,183,0.08);
                ...
                <span style="font-size:11px;color:#9a85aa;font-family:'Inter',sans-serif;">
            ...
            <div style="padding:0;">
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;
                    ...
                    <div style="flex:1;min-width:0;">
                        <div style="font-family:'Inter',sans-serif;font-size:13px;color:#1e0a2c;
                    ...
                    <div style="text-align:right;flex-shrink:0;margin-left:12px;">
                        <span style="font-family:'Inter',sans-serif;font-size:14px;font-weight:700;color:#b52020;">
    ...
    <div style="background:#fff;border:1px solid rgba(185,92,183,0.10);border-radius:4px;
        ...
        <div style="display:flex;align-items:center;gap:8px;">
            ...
            <span style="font-family:'Inter',sans-serif;font-size:13px;color:#2e1040;">
                Add Stock: <strong>{{ $mvtIn->count }}</strong> entries
                @if($mvtIn->total_cost > 0) · ₹{{ number_format($mvtIn->total_cost, 0) }} @endif
            </span>
        </div>
        <div style="display:flex;align-items:center;gap:8px;">
            <span style="width:8px;height:8px;background:#b52020;border-radius:50%;display:inline-block;"></span>
            <span style="font-family:'Inter',sans-serif;font-size:13px;color:#2e1040;">
                Use Item: <strong>{{ $mvtOut->count }}</strong> entries
                @if($mvtOut->total_cost > 0) · ₹{{ number_format($mvtOut->total_cost, 0) }} @endif
            </span>
        </div>
        <a href="{{ route('inventory.reports') }}"
           style="margin-left:auto;font-family:'Inter',sans-serif;font-size:12px;
...
@endif {{-- end non-doctor full dashboard --}}
        ...
        new Chart(trendCtx, {
            ...
            options: {
                ...
                scales: {
                    x: {
                        grid: { display: false },
                        ticks: { font: { size: 10, family: 'Inter' }, color: '#9a85aa' },
