...
<div style="display:flex;gap:2px;border-bottom:2px solid #f0e8f4;margin-bottom:20px;">
    <a href="?tab=catalog"
       style="padding:8px 18px;font-family:'Inter',sans-serif;font-size:13px;
        ...
        <tbody>
            ...
            <tr style="border-bottom:1px solid rgba(185,92,183,0.05);transition:background 120ms;"
                ...
                <td style="padding:10px 14px;">
                    <div style="font-weight:600;color:#1a0a1e;font-family:'Inter',sans-serif;">{{ $item->brand }}</div>
                ...
                <td style="padding:10px 14px;font-family:'Inter', sans-serif;font-size:12px;color:#4e2060;">
                    {{ $item->product_code ?? '—' }}
                    @if($item->description)<div style="font-size:10.5px;color:#9070a0;font-family:'Inter',sans-serif;">{{ Str::limit($item->description, 40) }}</div>@endif
                </td>
                <td style="padding:10px 14px;text-align:center;font-size:12px;color:#2e1040;">
                    ...
                    @endphp
                    @if(count($dims))
                        <span style="font-family:'Inter', sans-serif;font-size:11.5px;">{{ implode(' × ', $dims) }}</span>
                    @else
                        <span style="color:#9a85aa;">—</span>
                    @endif
                </td>
                <td style="padding:10px 14px;text-align:right;font-family:'Inter', sans-serif;font-size:13px;color:#1e0a2c;">
                    @if($item->unit_price)
        ...
        <tbody>
            ...
            <tr style="border-bottom:1px solid rgba(185,92,183,0.05);transition:background 120ms;"
                ...
                <td style="padding:11px 14px;">
                    ...
                    <div style="font-size:11px;color:#9a85aa;font-family:'Inter', sans-serif;">
                ...
                <td style="padding:11px 14px;font-family:'Inter', sans-serif;font-size:11.5px;">
                    @if($pl->lot_number)
    ...
    <div style="background:#fff;border-radius:8px;width:100%;max-width:640px;
        ...
        <form method="POST" action="{{ route('inventory.implants.catalog.store') }}"
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
                ...
                <div>
                    <label class="imp-label">Unit Price (Rs. )</label>
            ...
            <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:18px;
                ...
                <button type="submit"
                        style="background:#6a0f70;color:#fff;border:none;border-radius:6px;
                               padding:9px 18px;font-size:13px;font-family:'Inter',sans-serif;
    ...
    <div style="background:#fff;border-radius:8px;width:100%;max-width:660px;
        ...
        <div style="padding:18px 24px;border-bottom:1px solid rgba(26,122,69,0.12);
            ...
            <div>
                ...
                <p style="font-family:'Inter',sans-serif;font-size:12px;color:#5a8a6a;margin:4px 0 0;">
        ...
        <form method="POST" action="{{ route('inventory.implants.placements.store') }}"
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
                ...
                <div style="grid-column:1/-1;">
                    ...
                    <input type="hidden" name="patient_id" id="patient-id-val" required>
                    <input type="text" id="patient-search"
                           placeholder="Type name or phone to filter…"
                           autocomplete="off"
                           oninput="filterPatients(this.value)"
                           onfocus="document.getElementById('patient-list').style.display='block'"
                    ...
                    <div id="patient-list"
                         style="width:100%;max-height:160px;overflow-y:auto;border:1px solid #d8c8e4;
                                border-radius:0 0 5px 5px;background:#fff;box-sizing:border-box;font-family:'Inter',sans-serif;font-size:13px;">
                        <div data-pid="" data-search=""
                             onclick="selectPatient('','')"
                             style="padding:7px 10px;color:#9a85aa;cursor:pointer;border-bottom:1px solid #f5f0f8;">
                            — Select patient —
                        </div>
                        @forelse($patients as $pt)
                        <div data-pid="{{ $pt->id }}"
                             data-search="{{ strtolower($pt->name . ' ' . $pt->phone) }}"
                             onclick="selectPatient('{{ $pt->id }}','{{ addslashes($pt->name . ($pt->phone ? ' · '.$pt->phone : '')) }}')"
                             style="padding:7px 10px;cursor:pointer;border-bottom:1px solid #f9f6fc;"
                             onmouseover="this.style.background='#f0e8f4'" onmouseout="this.style.background=''">
                            {{ $pt->name }}{{ $pt->phone ? ' · ' . $pt->phone : '' }}
                        </div>
                        @empty
                        <div style="padding:7px 10px;color:#9a85aa;">No patients found.</div>
                        @endforelse
                    </div>
                    <script>
                ...
                <div style="grid-column:1/-1;">
                    ...
                    <div style="border:2px dashed #d8c8e4;border-radius:6px;padding:16px;
                        ...
                        <div style="font-size:13px;color:#9070a0;font-family:'Inter',sans-serif;">
                ...
                <div style="grid-column:1/-1;">
                    ...
                    <textarea name="notes" rows="2"
                              style="width:100%;padding:8px 10px;border:1px solid #d8c8e4;border-radius:5px;font-size:13px;font-family:'Inter',sans-serif;box-sizing:border-box;resize:vertical;"></textarea>
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:16px;">
                ...
                <div style="grid-column:1/-1;">
                    ...
                    <input type="file" name="label_photo" accept="image/*"
                           style="font-size:12px;font-family:'Inter',sans-serif;">
                </div>
                <div style="grid-column:1/-1;">
                    <label class="imp-label">Notes</label>
...
<div id="photo-viewer"
    ...
    <div style="font-size:12px;color:rgba(255,255,255,0.5);margin-bottom:12px;font-family:'Inter',sans-serif;"
         id="photo-viewer-title"></div>
    <img id="photo-viewer-img" src="" alt="" style="max-width:90vw;max-height:80vh;border-radius:4px;">
    <div style="margin-top:12px;font-size:11px;color:rgba(255,255,255,0.4);font-family:'Inter',sans-serif;">
