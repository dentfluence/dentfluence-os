...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px;">
    @php $stats = [
        ['label'=>'Total Stock Value','value'=>'Rs. '.number_format($totalValue,0),'color'=>'#6a0f70','bg'=>'#f9f3fa'],
    ...
    <div style="background:#fff;border:1px solid rgba(185,92,183,0.12);border-radius:4px;padding:16px;">
        <div style="font-family:'Inter', sans-serif;font-size:22px;font-weight:700;color:{{ $s['color'] }};">{{ $s['value'] }}</div>
        ...
        <div class="df-card-body">
            ...
            <div style="margin-bottom:12px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
                    ...
                    <span style="font-size:12px;color:#9a85aa;font-family:'Inter', sans-serif;">Rs. {{ number_format($cv->value,0) }} <span style="font-size:10px;">({{ $pct }}%)</span></span>
                </div>
        ...
        <div class="df-card-body">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;padding:9px 0;border-bottom:1px solid rgba(185,92,183,0.06);">
                ...
                <div style="text-align:right;">
                    <div style="font-family:'Inter', sans-serif;font-size:13px;font-weight:600;color:#1e0a2c;">{{ $mv->count }}×</div>
                    @if($mv->total_cost > 0)<div style="font-size:11px;color:#9a85aa;">Rs. {{ number_format($mv->total_cost,0) }}</div>@endif
                </div>
            </div>
            @empty
            ...
            <table style="width:100%;border-collapse:collapse;font-size:12.5px;">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid rgba(185,92,183,0.05);">
                        ...
                        <td style="padding:9px 14px;text-align:right;font-family:'Inter', sans-serif;color:#6a0f70;font-weight:600;">{{ number_format($item->total_qty,0) }} <span style="font-size:10px;color:#9a85aa;">{{ $item->consumption_unit }}</span></td>
                        <td style="padding:9px 18px;text-align:right;font-family:'Inter', sans-serif;color:#1e0a2c;">Rs. {{ number_format($item->total_cost,0) }}</td>
                    </tr>
                    @empty
...
new Chart(document.getElementById('dailyChart'), {
    ...
    data: {
        ...
        datasets: [
            { label: 'Stock In Value (Rs. )', data: daily.map(d => d.in), backgroundColor: 'rgba(26,122,69,0.7)', borderRadius: 2 },
            { label: 'Dispensed (count)', data: daily.map(d => d.out), backgroundColor: 'rgba(106,15,112,0.55)', borderRadius: 2, yAxisID: 'y1' }
        ]
    },
    options: {
        responsive: true, maintainAspectRatio: false,
        plugins: { legend: { labels: { font: { size: 11, family: 'Inter' }, boxWidth: 10 } } },
        scales: {
            x: { grid: { display: false }, ticks: { font: { size: 10 }, color: '#9a85aa' }, border: { display: false } },
            y: { grid: { color: 'rgba(185,92,183,0.06)' }, ticks: { font: { size: 10, family: 'Inter' }, color: '#9a85aa', callback: v => 'Rs. '+(v>=1000?(v/1000).toFixed(0)+'k':v) }, border: { display: false } },
