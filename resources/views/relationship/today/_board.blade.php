{{--
    TODAY'S ACTIONS — THE BOARD, used in two places.

    ── WHY A PARTIAL (23 Sep) ──────────────────────────────────────────────────
    This board renders at /relationship/today, at /relationship/today/pending,
    and now inline on My Day. It is the SAME board everywhere: the same W-10
    patient folding (one patient, one row, one call), the same call drawer, the
    same logging. A simplified copy on My Day would have been quicker and would
    have taught reception that the two screens mean different things — which is
    the confusion My Day exists to end.

    ── WHAT IT NEEDS ───────────────────────────────────────────────────────────
    All nineteen variables from TodayController::boardPayload(). That method is
    public for exactly this reason; do not rebuild the payload by hand.

    ── $compact ────────────────────────────────────────────────────────────────
    false (default) — the board as its own page, with its title and subtitle.
    true            — embedded: the title block is dropped because the host page
                      already says where you are. EVERYTHING ELSE STAYS — the
                      search, the category strip, the toolbar, the drawer. It is
                      a copy of the board, not a preview of it.

    ── IF YOU INCLUDE THIS TWICE ON ONE PAGE ───────────────────────────────────
    Do not. CHECKLISTS, RESPONSE_OPTS, todayActions() and taWorklist() are
    declared at top level; a second copy redeclares a const and kills the whole
    page's JavaScript, not just the second board. The same applies to including
    it beside tasks/_board.blade.php — those are different names, so they may
    share a page. One copy of each.
--}}
@php
    $compact = $compact ?? false;
@endphp

{{-- The stylesheet travels with the board. My Day extends layouts.app, which
     has no head-extra of its own to fill, so pushing to the 'styles' stack is
     the only way the board can look like itself off its own page. @once keeps
     it to a single copy however many times Blade walks this file. --}}
@once
@push('styles')
...
>
    ...
    <div class="taw" x-data="taWorklist(window.__TA_ROWS || [])">
        ...
        @unless($compact)
        <div class="taw-head">
            {{-- The host page already says where you are. A second
                 <h1> under My Day's own heading reads as a different
                 page beginning, which is exactly what this page is
                 trying to stop. --}}
        ...
        @if($activeGroups->isNotEmpty() && ! $compact)
        <div class="taw-cats">
        ...
        @if(empty($rows) && $compact)
        <div style="padding:9px 12px;font-size:12.5px;color:#8a7a92;background:#fdfbfe;border:1px solid #ece2f1;border-radius:9px;">
            No calls due today.
        </div>
        @elseif(empty($rows))
