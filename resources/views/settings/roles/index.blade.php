            ...
            <div style="margin-bottom:14px;">
                ...
                <div style="display:flex;gap:8px;">
                    <label :style="newRole.category==='staff' ? 'border-color:#6a0f70;background:#faf0ff;' : 'border-color:#ddd;background:#fff;'"
                        ...
                        <input type="radio" x-model="newRole.category" value="staff" style="accent-color:#6a0f70;">
                        Staff
                    </label>
                    <label :style="newRole.category==='doctor' ? 'border-color:#1a7a45;background:#f2fbf5;' : 'border-color:#ddd;background:#fff;'"
                           style="flex:1;display:flex;align-items:center;gap:7px;padding:8px 12px;border:1.5px solid #ddd;border-radius:6px;cursor:pointer;font-size:13px;color:#1a0320;transition:all 120ms;">
                        <input type="radio" x-model="newRole.category" value="doctor" style="accent-color:#1a7a45;">
                        Doctor
                    </label>
                </div>
            </div>
...
function rolesManager() {
    return {
        ...
        // All permissions pre-loaded from server — no async fetch needed
        ...
        selectRole(id, name, slug) {
            ...
            this.savedMsg         = false;
            // Load from pre-loaded data (instant, no fetch)
        ...
        async savePermissions() {
            ...
            try {
                ...
                // Update in-memory cache so other role tabs stay accurate
