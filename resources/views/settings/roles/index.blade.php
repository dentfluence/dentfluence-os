...
<div
    x-data="rolesManager()"
    x-init="init()"
    style="font-family:'DM Sans',sans-serif; padding: 28px 32px; max-width: 1200px;"
>
    ...
    <div style="display:grid; grid-template-columns:260px 1fr; gap:20px; align-items:start;">
        ...
        <div style="background:#fff; border:1px solid #ede4f3; border-radius:10px; overflow:hidden;">
            <div style="padding:14px 16px; border-bottom:1px solid #ede4f3; font-size:11px; font-weight:600; letter-spacing:.15em; text-transform:uppercase; color:#9a7aaa;">
                Roles
            </div>

            @foreach($roles as $role)
            <div
                @click="selectRole({{ $role->id }}, '{{ $role->name }}', '{{ $role->slug }}')"
            ...
            >
                <div style="display:flex; align-items:center; gap:10px; padding:13px 16px; border-bottom:1px solid #f5f0f8; transition:background 120ms;">
                    ...
                    <div style="flex:1; min-width:0;">
                        <div style="font-size:13.5px; font-weight:500; color:#1a0320; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                            {{ $role->name }}
                        </div>
                        <div style="font-size:11px; color:#9a7aaa; margin-top:1px;">
                            {{ $role->users_count }} {{ Str::plural('staff', $role->users_count) }}
                            @if($role->is_system)
                            · <span style="color:#b0a0bb;">system</span>
                            @endif
                        </div>
                    </div>

                    {{-- Active indicator --}}
                    <svg x-show="selectedRoleId === {{ $role->id }}"
                         width="14" height="14" viewBox="0 0 24 24" fill="none"
                         stroke="#6a0f70" stroke-width="2.5">
        ...
        <div style="background:#fff; border:1px solid #ede4f3; border-radius:10px; overflow:hidden;">
            ...
            <div style="display:flex; align-items:center; justify-content:space-between; padding:14px 20px; border-bottom:1px solid #ede4f3;">
                ...
                <button
                    x-show="selectedRoleId"
                    @click="savePermissions()"
                    :disabled="saving"
                    style="display:inline-flex; align-items:center; gap:6px; padding:7px 16px; background:#6a0f70; color:#fff; border:none; border-radius:6px; font-size:12.5px; font-weight:500; cursor:pointer; opacity:1; transition:opacity 150ms;"
            ...
            <div x-show="selectedRoleId" x-cloak>
                ...
                {{-- Loading state --}}
                <div x-show="loadingPerms" style="padding:40px; text-align:center; color:#9a7aaa; font-size:13px;">
                    Loading permissions…
                </div>
                ...
                <div x-show="!loadingPerms">
                    ...
                    <div style="display:grid; grid-template-columns:1fr 90px 90px 90px; gap:0; padding:11px 20px; border-bottom:1px solid #f5f0f8; align-items:center;"
                        ...
                        <div style="display:flex; align-items:center; gap:8px;">
                            ...
                            <span style="font-size:13px; color:#1a0320; font-weight:400;">{{ $module->name }}</span>
                        </div>
                        ...
                        <div style="text-align:center;">
                            <label class="df-toggle" :class="permissions['{{ $module->slug }}']?.view ? 'on' : ''">
                                <input type="checkbox"
                                       :checked="permissions['{{ $module->slug }}']?.view"
                                       @change="toggle('{{ $module->slug }}', 'view', $event.target.checked)"
                                       style="display:none;">
                                <span class="df-toggle-track"></span>
                            </label>
                        </div>

                        {{-- Edit toggle (disabled if no view) --}}
                        <div style="text-align:center;">
                            <label class="df-toggle" :class="permissions['{{ $module->slug }}']?.edit ? 'on' : ''"
                                ...
                                <input type="checkbox"
                                       :checked="permissions['{{ $module->slug }}']?.edit"
                        ...
                        {{-- Delete toggle (disabled if no edit) --}}
                        <div style="text-align:center;">
    ...
    <div x-show="showNewRoleModal"
        ...
        <div style="background:#fff;border-radius:12px;width:420px;padding:28px;box-shadow:0 20px 60px rgba(14,1,24,.25);">
            <h2 style="font-family:'Cormorant Garamond',serif;font-size:20px;font-weight:700;color:#1a0320;margin:0 0 20px;">
                Create New Role
            </h2>
            ...
            <div style="margin-bottom:14px;">
                ...
                <input x-model="newRole.description" type="text" placeholder="Brief description of this role"
                       style="width:100%;padding:9px 12px;border:1.5px solid #ddd;border-radius:6px;font-size:13px;outline:none;box-sizing:border-box;"
                       @focus="$el.style.borderColor='#6a0f70'" @blur="$el.style.borderColor='#ddd'">
            </div>
            ...
            <div style="display:flex;gap:10px;justify-content:flex-end;">
                ...
                <button @click="createRole()"
                        ...
                        :style="!newRole.name ? 'opacity:.5;cursor:not-allowed;' : ''">
                    Create Role
                </button>
            </div>
        </div>
...
<style>
    ...
    .df-toggle-track::after {
        ...
        background:#fff; transition:transform 180ms, background 180ms;
...
function rolesManager() {
    return {
        ...
        selectedRoleName: '',
        selectedRoleSlug: '',
        permissions:      {}, // { module_slug: { view, edit, delete } }
        loadingPerms:     false,
        saving:           false,
        savedMsg:         false,
        showNewRoleModal: false,
        newRole:          { name:'', description:'', color:'#6a0f70' },
        colours: ['#6a0f70','#1a5ea8','#1a7a45','#a05c00','#c0392b','#2c3e50','#8e24aa','#00695c','#ad1457'],

        init() {
            // Auto-select first role
            const firstId = {{ $roles->first()?->id ?? 'null' }};
            if (firstId) {
        ...
        toggle(slug, action, value) {
            ...
            // Cascade: turning on edit requires view
            if (action === 'edit' && value) {
                this.permissions[slug].view = true;
            }
            // Cascade: turning on delete requires edit + view
        ...
        async savePermissions() {
            this.saving = true;
            try {
                await fetch(`/settings/roles/${this.selectedRoleId}`, {
                    method:  'POST',
                    headers: {
